  ...
  return (
    <div className='flex flex-col gap-4'>
      {projects.map(({ title, description, tags, image, video, link }) => (
        <Card key={title} className='flex flex-col border shadow w-full max-w-4xl mx-auto'>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  ...
                  <a
                    ...
                    target='_blank'
                    rel='noreferrer'
                    className='inline-flex items-center group gap-2 transition-all duration-300 hover:scale-[1.02] hover:text-blue-600 dark:hover:text-blue-400'
                  >
          ...
          <CardContent className='flex flex-col space-y-6 p-4 pt-0'>
            <div className='flex flex-wrap gap-2'>
              {tags.map((tag) => (
                <Badge
                  className='p-1 gap-1 rounded-md shadow transition-all duration-300 hover:scale-[1.05] hover:shadow-md dark:hover:bg-neutral-800 hover:bg-neutral-100 cursor-pointer'
                  variant='secondary'
                  key={tag.name}
                >
            ...
            <div className='flex gap-x-2'>
              {link.preview && (
                ...
                variant='default'
                size={null}
                className='p-2 shadow transition-all duration-300 bg-neutral-900 text-white hover:scale-[1.03] hover:shadow-md hover:bg-neutral-800 dark:bg-neutral-100 dark:text-black dark:hover:bg-neutral-200'
                asChild
              >
                <a
                  href={link.preview}
